...
def _build_pyramid(image, levels):
    """A "pyramid" is an image, the same image at 1/2 the size, at 1/4, etc.
...
def _log_image_descriptions(
    ...
    template = jinja2.Template("""
        ...
        {% for level in levels %}
            ...
            {% if level == 0 %}
                <li>Matched at {{position}} with certainty
                    {{"%.2f"|format(first_pass_certainty)}}.
            {% else %}
                <li>Didn't match (best match at {{position}} with certainty
                    {{"%.2f"|format(first_pass_certainty)}}).
            {% endif %}
