...
test_stbt_motiondetect_with_high_noisethreshold_reports_motion() {
    run_motiondetect \
        "videotestsrc" \
        "enabled=true noiseThreshold=1.0" \
        "$scratchdir/gst-launch.log"
    check_expectations_from_log \
        $EXPECT_RESULT $EXPECT_MOTION "$scratchdir/gst-launch.log"
}

test_stbt_motiondetect_with_low_noisethreshold_does_not_report_motion() {
    run_motiondetect \
        "videotestsrc" \
        "enabled=true noiseThreshold=0.0" \
        "$scratchdir/gst-launch.log"
    check_expectations_from_log \
        $EXPECT_RESULT $DONT_EXPECT_MOTION "$scratchdir/gst-launch.log"
}

test_stbt_motiondetect_reports_motion_on_progress_dots() {
    run_motiondetect \
        "multifilesrc location=motiondetect-dots-%d.png index=1 \
            caps=image/png,framerate=30/1 ! decodebin2" \
        "enabled=true" \
        "$scratchdir/gst-launch.log"
    check_expectations_from_log \
        $EXPECT_RESULT $EXPECT_MOTION "$scratchdir/gst-launch.log"
...
# parameters; "gst-launch --messages" logs the bus messages generated by
...
# from the second frame; "gst-launch --messages" logs the bus messages; and
